...
class SeanceRepository extends ServiceEntityRepository
{
    ...
    public function findBySelection($defaultclasse, $defaultmatiere, $defaultheure){
        $query = $this->createQueryBuilder('a');
        if ($defaultclasse){
            $query
                ->andWhere('a.nomclasse = :nomclasse')
                ->setParameter('nomclasse', $defaultclasse);
        }
        if ($defaultmatiere){
            $query
                ->andWhere('a.matiere = :matiere')
                ->setParameter('matiere', $defaultmatiere);
        }
        if ($defaultheure){
            $query
                ->andWhere('a.heure = :heure')
                ->setParameter('heure', $defaultheure);
        }
        return $query
            ->getQuery()
            ->getResult();
